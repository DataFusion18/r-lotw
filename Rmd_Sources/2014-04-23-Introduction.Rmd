...
Lessons will go out on Monday mornings and homework will be due the following Monday at noon (local time). The weekly lessons will come as attache .html files. For best viewing, download these files to your local machine and open in Chrome. If you open by directly clicking on the link from within gmail, the .html file will not render with full formatting.

## Installing R and RStudio

R can be installed from the [CRAN website](http://cran.us.r-project.org/). Windows users working under enterprise security restrictions, should work with their IT staff to install R in a location that allows proper write permissions. For AFSC users, NMML IT staff have instructions for installing R, RStudio, MiKTeX and RTools in a manner that is compatible with our networking and securtiy settings. The most important aspect of the install for AFSC windows users is to, prior to installing any of the software, create a Windows system environment variable R_LIBS and set the value equal to the `C:\Users\user.name\Rlibs`. This will force R to install and manage any libraries and packages within a directory that you have write permissions to.

OS X users should be able to install R with typical settings and less headache.

[RStudio (preview release)](http://www.rstudio.com/ide/download/preview) is an Integrated Development Environment (IDE) developed for R. RStudio is cross-platform and should be installed using default settings for all platforms. This provides a more capable software interface for developing R scripts, creating plots, knitr documents, version control, package management, etc. All lessons will presume the use of RStudio as the IDE. You must install the RStudio Preview Release linked to above for full compatibility.
